...
- Write to `[logs_folder]/YYYY/MM/YYYY-MM-DD-session-NN.md` using the Session Log Format from `references/session-formats.md` (variant: Auto-saved — frontmatter adds `auto-saved: true`; add `synthesized_from_checkpoints: true` only if checkpoints were found and incorporated). **Do not write the session log if any unmerged checkpoint's content is absent from the relevant sections** — every checkpoint's Key Decisions, Action Items, and Open Questions must appear explicitly in the output.
