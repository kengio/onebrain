...
import type { VaultConfig, DoctorResult } from './index.js';
...
describe('checkHarnessBinary', () => {
  it('returns ok for "direct" harness without checking PATH', async () => {
    const config: VaultConfig = {
      folders: {
        ...
        knowledge: '03-knowledge', resources: '04-resources', agent: '05-agent',
        archive: '06-archive', logs: '07-logs',
      },
      checkpoint: { messages: 15, minutes: 30 },
      update_channel: 'stable',
  ...
  it('returns ok when claude-code binary is found', async () => {
    ...
    expect(result.check).toBe('harness-binary');
