...
| `/wrapup` | Wrap up session and save summary to memory log |
...
One file per session: `YYYY-MM-DD-session-NN.md`. Contains summaries, decisions, insights, and action items. Generated by `/wrapup` at the end of each session. Default folder name is `04-memory-log/`; may differ based on your vault method (see `vault.yml`).
