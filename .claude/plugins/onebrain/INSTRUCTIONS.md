...
After creating or editing a content note (any file outside `.claude/`, `[agent folder]/`, `[logs folder]/`, and `attachments/`), include a markdown link at the end of your response so the user can open it in Obsidian on demand:

```
[Open in Obsidian](obsidian://open?path=/absolute/path/to/file.md)
```
...
- Checkpoints: `07-logs/YYYY/MM/YYYY-MM-DD-checkpoint-NN.md` (auto-generated by hooks, not manual)
