...
Vault folders are created during `/ob:onboarding`. You'll choose one of three organization methods — each creates a different folder layout and updates all system files to match.
...
The core workflow: capture everything to inbox → process with `/ob:consolidate` → grow your knowledge base → archive what's done.
...
Skills that save reference notes (like `/ob:research`, `/ob:consolidate`, and `/ob:capture`) route to `03-resources/`. The `02-areas/` folder is for manually organizing ongoing responsibilities — there is no automated skill routing to it.
...
|---------|-------------|
| `/ob:onboarding` | First-run setup — run this first |
| `/ob:braindump` | Dump everything on your mind — it gets classified and filed |
| `/ob:capture` | Quick note with auto-linking to related notes |
| `/ob:consolidate` | Process inbox into permanent knowledge |
| `/ob:connect` | Find connections between notes, suggest wikilinks |
| `/ob:research [topic]` | Web research → structured note in your vault |
| `/ob:summarize-url [url]` | Fetch a URL and save a summary note |
| `/ob:reading-notes` | Turn a book or article into structured notes |
| `/ob:weekly` | Review the week, surface patterns, set intentions |
...
One file per session: `YYYY-MM-DD-session-NN.md`. Contains summaries, decisions, insights, and action items. Generated by `/ob:wrapup` at the end of each session. Default folder name is `04-memory-log/`; may differ based on your vault method (see `vault.yml`).
...
After `/ob:onboarding`, your AI agent:
