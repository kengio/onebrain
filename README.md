...
└── 05-logs/           Session logs (one per session, organized by YYYY/MM)
```
...
Note: `00-inbox/` and `05-logs/` are OneBrain additions on top of the PARA taxonomy — they are not native PARA categories.
...
One file per session: `YYYY-MM-DD-session-NN.md`, organized into `YYYY/MM/` subfolders. Contains summaries, decisions, insights, and action items. Generated by `/wrapup` at the end of each session. Default folder name is `04-logs/`; may differ based on your vault method (see `vault.yml`).
