...
**Resolve the resources folder first:** read `vault.yml` for `folders.resources`, defaulting to `[resources_folder]`. Store as `[resources_folder]`.
...
1. Resolve file path using `[resources_folder]` from Step 2 if in the same session, otherwise read `vault.yml` for `folders.resources` (default `[resources_folder]`)
