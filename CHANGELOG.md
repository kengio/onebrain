...
- fix(orphan-scan): malformed vault.yml now writes a one-line warning to stderr (parse errors, non-mapping root, EACCES) so the user can discover their config is being silently ignored. Expected absence (`vault.yml not found`) stays silent; the prefix-match classifier prevents a stranger error message that happens to contain the substring from slipping through.
- fix(orphan-scan): `runOrphanScan` rejects empty `vaultRoot` with a clear error message — empty string would resolve `vault.yml` against `process.cwd()` and silently consume an unrelated vault config. Programming-bug guard for future programmatic callers; the CLI wrapper always passes `process.cwd()`.
- fix(types): `VaultConfig.checkpoint` is now non-optional. The parser unconditionally constructs it from `DEFAULT_CHECKPOINT`; the `?` modifier was vestigial and forced consumers to write defensive `?.minutes` chains the runtime never needed.
- test(orphan-scan): 18 new cases — boundary 30/60/90 min, newest-mtime-wins, future-mtime fail-safe, cross-month globally-newest, mixed stale+active groups, threshold scaling for `checkpoint.minutes` ∈ {15, 30, 60}, malformed/missing vault.yml fallback, and the new stderr-warn + empty-vaultRoot input contracts.
