...
2. Use `PPID` from context if already loaded; if absent, run the session token command via Bash and save to context. The session token is alphanumeric — numeric on Mac/Linux (`$PPID`), alphanumeric on Windows Terminal (`$WT_SESSION` prefix).
